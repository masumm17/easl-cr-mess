...
class CR_Dropdown_Walker_Nav_Menu extends Walker_Nav_Menu {
	 ...
	 */
	public function start_lvl( &$output, $depth = 0, $args = array(), $total_childs = 0 ) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$bg_style = '';
		if(1 == $depth){
			$bg_style = $this->menu_item_image($item->ID);
		}
		$output .= $indent . '<li' . $id . $class_names . $bg_style . '>';
		if(1 == $depth){
			$output .= '<div class="cr-menu-item-content">';
		}
	...
	public function display_element( $element, &$children_elements, $max_depth, $depth, $args, &$output ) {
		...
		if($this->has_children) {
			$element->classes = empty( $element->classes ) ? array() : (array) $element->classes;
			$total_childs = count($children_elements[$id]);
			$total_childs = min(6, $total_childs);
			if($depth < 1){
		...
		if ( ($max_depth == 0 || $max_depth > $depth+1 ) && isset( $children_elements[$id]) ) {
			...
			foreach ( $children_elements[ $id ] as $child ){
				...
				if ( !isset($newlevel) ) {
					...
					$cb_args[] = $total_childs;
					call_user_func_array(array($this, 'start_lvl'), $cb_args);
				}
				$this->display_element( $child, $children_elements, $max_depth, $depth + 1, $args, $output );
				if($depth == 0 && $count_childs == 6) {
					break;
				}
			}
