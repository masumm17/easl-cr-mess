...
if ( ! function_exists( 'cr_theme_posted_on' ) ) :
	...
	function cr_theme_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'crt' ),
...
if ( ! function_exists( 'cr_theme_posted_by' ) ) :
	...
	function cr_theme_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'crt' ),
	...
	function cr_theme_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'crt' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'crt' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'crt' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'crt' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'crt' ),
					array(
						'span' => array(
							'class' => array(),
						),
